...
export class ConfiguracionComponent implements OnInit {
  ...
  constructor(config: NgbModalConfig,private servicio: BackenApiService,private modal:NgbModal,private toastr: ToastrService) {
  ...
  botonCrearNuevoEstado(){
    ...
    if(this.nombreEstadoCtrl.value!='' && this.selectIDTipEstadoModal!=0  && this.banderaSelectEstado==false){

      this.NotificacionEstadoCreado()
        
      this.limpiarModalEstado() 
    }else if(this.nombreEstadoCtrl.value!='' && this.selectIDTipEstadoModal==0 && this.banderaSelectEstado==true && this.nombreTipoEstadoCtrl.value!=''){
  ...
  getTipoEstado(): void {
    this.servicio.getTipoEstadoAdmin(this.IDRol).subscribe(
      (res) => {
        ...
        this.objModalTipoEstado=res; /* para rellenar el select del modal tipo estado */
      },
  ...
  NotificacionTipoReclamoCreado(){
    this.toastr.success(
      'El tipo de Reclamo a sido creado!',
      'Atención',
      {
        timeOut:2000,
        positionClass:'toast-bottom-center',   
